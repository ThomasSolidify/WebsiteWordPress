...
function bespoke_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'bespoke' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'bespoke_custom_background_args',
			array(
 ...
 */
function bespoke_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'bespoke_content_width', 640 );
}
add_action( 'after_setup_theme', 'bespoke_content_width', 0 );
...
function bespoke_scripts() {
	wp_enqueue_style( 'bespoke-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'bespoke-style', 'rtl', 'replace' );

	wp_enqueue_script( 'bespoke-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
